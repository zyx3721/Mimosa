...
> **每台设备的网卡名不同，不修改则网络速度/流量显示为空或报错。**

| 文件 | 位置 | 说明 |
|------|------|------|
| `Mimosa.conf` | 第 84～88 行 | 共出现 **6 处** `wlp0s20f3`，分别对应 `downspeed`、`totaldown`、`downspeedgraph`、`upspeed`、`totalup`、`upspeedgraph` |
...
nmcli device status
```
找到你的无线网卡（通常以 `wl` 开头）或有线网卡（通常以 `en` 或 `eth` 开头），将 `wlp0s20f3` 全部替换为你的网卡名。
